...
export default function ModalAñadirCarta({ carta, sets = [], onConfirm, onCancel }) {
  ...
  const findSetByKey = (raw) => {
    ...
    const s = sets.find((st) =>
      (st.id && st.id.toLowerCase() === key) ||
      (st.localId && st.localId.toLowerCase() === key) ||
      (st.code && st.code.toLowerCase() === key)
    );
  ...
  const getSetName = () => {
    // 1) Si viene objeto set completo
    const setObj =
      tcg?.set ||
      carta?.tcgdex?.set ||
      carta?.data?.set ||
    ...
    // 2) tcg.set como string
    if (typeof tcg?.set === 'string') {
      const fromId = findSetByKey(tcg.set);
      if (fromId) return fromId;
      return tcg.set;
    }

    // 3) carta.set como string
    if (typeof carta?.set === 'string') {
      const fromId = findSetByKey(carta.set);
      if (fromId) return fromId;
      return carta.set;
    }

    // 4) Derivar código de set desde id de carta
    ...
    // 5) Intentar deducirlo desde la URL de la imagen
    ...
    if (candidateImageUrl && Array.isArray(sets) && sets.length > 0) {
      try {
        const url = new URL(candidateImageUrl);
        const segments = url.pathname.split('/').filter(Boolean);
        for (const seg of segments) {
          const fromSeg = findSetByKey(seg);
